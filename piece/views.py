...
class GenreDetail(APIView):
    ...
    def get_object(self, isbn):
        try:
            return Genre.objects.get(isbn=isbn)
        except Genre.DoesNotExist:
            return None

    def get(self, request, isbn):
        genre = self.get_object(isbn)
        if genre:
...
class PublisherDetail(APIView):
    ...
    def get_object(self, isbn):
        try:
            return Publisher.objects.get(isbn=isbn)
        except Publisher.DoesNotExist:
            return None

    def get(self, request, isbn):
        publisher = self.get_object(isbn)
        if publisher:
...
class PageListCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        if content_type == 'text':
            text_content_serializer = TextContentSerializer(data=data['content']['text_content'])
            ...
            image_content_serializer = ImageContentSerializer(data=data['content']['image_content'])
